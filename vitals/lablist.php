...
$query_lablist = sprintf("SELECT service.Short      , visit_procedure.Enrolee      , visit_procedure.Created      , visit_procedure.Creator      , visit_procedure.Visit      , visit_procedure.Provider      , visit_procedure.Appointment      , visit_procedure.Specific_Request      , visit_procedure.Specimen      , visit_procedure.Status      , procedure.Procedure   ,procedure.Short as dshort   , service.Task_FK     , task.Task     , task.Sequence     , procedure.Procedure     , procedure.Id     , service.Service     , task.Id     , visit_procedure.Status FROM     newmed06.procedure     INNER JOIN newmed06.service          ON (procedure.Service_direction_FK = service.Id)     INNER JOIN newmed06.task          ON (service.Task_FK = task.Id)     INNER JOIN newmed06.visit_procedure          ON (visit_procedure.Procedure = procedure.Id)         WHERE Enrolee = %s AND visit_procedure.Status = 1 ", GetSQLValueString($colname_lablist, "int"));
...
mysql_select_db($database_localhost, $localhost);
$query_lablisttwo ="SELECT service.Short      , visit_procedure.Enrolee      , visit_procedure.Created      , visit_procedure.Creator      , visit_procedure.Visit      , visit_procedure.Provider      , visit_procedure.Appointment      , visit_procedure.Specific_Request      , visit_procedure.Specimen      , visit_procedure.Status      , procedure.Short as dshort     , service.Task_FK     , task.Task     , task.Sequence     , procedure.Procedure     , procedure.Id     , service.Service     , task.Id     , visit_procedure.Status FROM     newmed06.procedure     INNER JOIN newmed06.service          ON (procedure.Service_direction_FK = service.Id)     INNER JOIN newmed06.task          ON (service.Task_FK = task.Id)     INNER JOIN newmed06.visit_procedure          ON (visit_procedure.Procedure = procedure.Id) WHERE visit_procedure.Status > 1 AND Enrolee = $colname_lablisttwo AND visit_procedure.Visit = '$colname2_lablisttwo' ";
...
<table width="100%" border="1" style="border:thin; border-color:#000; border-collapse:collapse;">
  <tr>
    <td width="80%"><strong>Laboratory</strong></td>
    <td width="20%">&nbsp;</td>
  </tr>
