...
$query_service = "SELECT * FROM service WHERE Direction = 1";
...
<body>
<form id="form201" name="form201" method="post" action="serviceinsert.php">
<table width="100%" bgcolor="#e5e5e5" style="border:thin; border-color:#000; border-collapse:collapse;" class="sample">
  <tr bgcolor="#b0b0b0" >
  <td  align="center"><strong>%</strong></td>
   <td align="center" class="header">Service</td>

  </tr>
  <?php do { ?>
  
  <input name="Service" type="hidden" value="<?php echo $row_service['Service']; ?>"/>
  <input name="Id" type="hidden" value="<?php echo $row_service['Id']; ?>"/>
    ...
    <tr bgcolor="#e5e5e5">
	  ...
	  if ($sch >= 0)
		
		{
			...
			?>
       <?php
		$colname_loading = $row_service['Id'];
...
mysql_select_db($database_localhost, $localhost);
$query_loading = sprintf("SELECT * FROM service_loading WHERE Service_FK = %s AND Effective_FK = %s", GetSQLValueString($colname_loading, "int") ,GetSQLValueString($colname2_loading, "int"));
...
$totalRows_loading = mysql_num_rows($loading);
      ?>
      <td width="14%" align="left"><input name="Loading" type="text" <?php echo $disable; ?>  <?php echo "onchange=\"AddLoad(this.value,'$_GET[ls]','$_GET[ef]','$_GET[sc]','$_SESSION[License]','$row_loading[Id]','$row_service[Service]','$row_service[Id]','serviceinsert.php','service.php');\"" ;?> value="<?php echo money($load = $row_loading['Loading']); ?>" style="width:100%; height:100%; text-align:right;" onBlur="this.value=formatCurrency(this.value);"/>        <input name="Id2" type="hidden" value="<?php echo $load = $row_loading['Id']; ?>" /></td><input name="Load" type="hidden" value="<?php echo $load; ?>" style="width:100%"/>
      <td width="86%" align="left">&nbsp;&nbsp;&nbsp;<?php echo $row_service['Service']; ?></td>
    </tr>
