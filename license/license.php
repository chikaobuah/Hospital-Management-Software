...
<div id="Licensehq">
   ...
   <table  class="sample">
        <tr class="header">
          <td width="20%" height="20px">Hospital </td>
        </tr>
    </table>
        <div  align="left"id="cont" style=" width : auto; overflow : auto; background-color:#e5e5e5">
          <?php 
...
FROM
    ...
    INNER JOIN newmed06.licensee 
        ON (licensee_hqs.Licensee = licensee.Id)WHERE Id <> 1 order by Id";
...
echo "<td >";
echo "<a class=\"linkr\"  onClick=\"getData('hq.php?licenhq=$ID','subscrb.php?lic=$ID')\">"   . $Licensee ."</a></li>\n";
...
FROM
	...
	INNER JOIN newmed06.state 
        ON (licensee.State_FK = state.Id)  WHERE licensee.Id = 2";
...
while($rowhq=mysql_fetch_array($resulthq))
{
  $lhqn =$rowhq['Licensee'];
      ...
      ?> 
        ...
        <div style="  height:30px; margin-top:115%"> 
		<?php 
		//echo "<a href=licensecreation.php title=\"Add new Licensee\" 
		//rel=\"gb_page_center[500,470]\" ><font color=\"#009\">Add New Licensee</font> </a>" ?></div>
    </div>
   ...
   <div style="background: url(../images/nav-bg.png) repeat-x; width:100px; height:100%; float:left;"><?php echo "<a class=\"links\" onClick=\"getData('branch.php?licenhq=$idpk')\">"   . "Licensee" ."</a></li>\n"; ?></div>
    </div>
    <div id="contact1">
