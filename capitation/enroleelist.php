...
<table width="100%" border="0"bgcolor="#e5e5e5" style="border:thin; border-color:#FFF; border-collapse:collapse;">
 <tr>
    <?php 
...
*/
  ...
  <tr>
    <td bgcolor="#b0b0b0" colspan="2" class="header" ><?php echo $row_count['count']; ?>&nbsp;Enrolee(s) &nbsp;(<?php echo $row_cap['Paying']; ?>)&nbsp;Total =&nbsp;<?php echo $total = $row_cap['Paying']*$row_count['count']; ?></td>
  </tr>
  <?php 
...
if ($totalRows_loading == 0){$sta = '0';  $id2 = '-1';}else{
$sta = $row_loading['Sta']; $id2 = $row_loading['Id'];}

$colname_schemepay2 = "-1";
if (isset($_GET['sc'])) {
  $colname_schemepay2 = $_GET['sc'];
}
mysql_select_db($database_localhost, $localhost);
$query_schemepay2 = sprintf("SELECT Id FROM scheme_payment WHERE Scheme_FK = %s AND Id = $id  AND Start_date > '$today' ", GetSQLValueString($colname_schemepay2, "int"));
$schemepay2 = mysql_query($query_schemepay2, $localhost) or die(mysql_error());
$row_schemepay2 = mysql_fetch_assoc($schemepay2);
$totalRows_schemepay2 = mysql_num_rows($schemepay2);


  
   if ($totalRows_schemepay2 > 0){
  ...
  <tr height="5px" class="tablebody">
	 ...
	 if ( $totalRows_Recordset22 == $row_count['count']) {
    ...
    <td width="1%" align="left" bgcolor="#e5e5e5"><input type="checkbox" <?php echo $check; ?>  value="<?php echo $value; ?>" name="che"  <?php echo $disable; ?> id="checkbox" <?php echo "onClick=\"AddAll(this.value,$id,$co,$sc,$pl,$che,$id2,$sta,$license,$enrolee);\" "?>/><strong>Select all</strong></td> 
  </tr>
	...
	<?php if ($row_loading['Status'] == 1)
	  {
		  if ($totalRows_Recordset22 > 0) {
		   echo "<input type=\"checkbox\" name=\"che\" id=\"checkbox\" $disable checked=\"checked\" value=\"1\" onClick=\"AddEnl($id,$co,$sc,$pl,$che,$id2,$sta,$license,$enrolee);\" />"; }
	  }
	  else 
	  { 
	  if ($totalRows_Recordset22 > 0) {
	 echo "<input type=\"checkbox\" name=\"che\" id=\"checkbox\" value=\"1\" $disable onClick=\"AddEnl($id,$co,$sc,$pl,$che,$id2,$sta,$license,$enrolee );\" />"; }
		  }
		   ?>
    
    
    
    <font size="-2"><?php echo ucfirst(strtolower($row_Recordset22['Surname'])); ?>&nbsp;<?php echo ucfirst(strtolower($row_Recordset22['First_Name'])); ?>&nbsp;<?php echo ucfirst(strtolower($row_Recordset22['Other_Name'])); ?>&nbsp;</font></td>
