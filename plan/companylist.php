...
mysql_select_db($database_localhost, $localhost);
$query_company = " SELECT  client.LId, client.Client     , client.Short     , client_service.Service_FK FROM newmed06.client_service     INNER JOIN newmed06.service          ON (client_service.Service_FK = service.Id)     INNER JOIN newmed06.client          ON (client_service.Client_FK = client.LId) WHERE client_service.Service_FK = $colname2_company AND  client.License = $colname_company ORDER BY client.Short";
...
<table  class="sample">
  <tr class="header">
    <td align="center" class="header">Provider <?php //echo $row_recpro['Program']; ?></td>
  </tr>
  <?php 
  do { 
  
  if ($totalRows_company != 0){$pro2 = $row_company['LId'];} else { $pro2 = '-1' ;} 
  
  if ($pro2 == $_GET['co']) { $color = "#84B8D0"; }  else  {$color = "#e5e5e5"; }
...
?>
      ...
      <td><?php 
	  $co =$row_company['LId']; 
	  echo "<a href=\"javascript:second('$co','$pl','$sc','$id');\">";  
	  echo $row_company['Short']; ?></td>
    </tr>
