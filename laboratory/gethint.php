...
AND visit_queue.Next_task IS NULL AND enrolee.Licensee = $session AND visit_queue.Task = 19
...
WHERE visit.Created LIKE '%". $colname_reclist2 ."%'
AND visit_queue.Next_task IS NOT NULL AND enrolee.Licensee = $session AND visit_queue.Task = 19
AND enrolee.Surname LIKE '%" . $q . "%'
GROUP BY enrolee.LId 
ORDER BY visit.Status ASC";
...
AND visit_queue.Next_task IS NULL AND enrolee.Licensee = $session AND visit_queue.Task = 19 AND enrolee.LId = $thisid
GROUP BY enrolee.LId 
ORDER BY visit.Status ASC";
